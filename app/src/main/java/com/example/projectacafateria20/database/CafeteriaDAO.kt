...
interface CafeteriaDAO {
    ...
    @Query("SELECT * FROM usuari WHERE name =:nombre AND password=:passw")
    fun getUsuariLogin(nombre:String, passw: String): LiveData<List<Usuari>>
    ...
    @Query("SELECT * FROM usuari")
    fun getPlat(): LiveData<List<Plat>>
